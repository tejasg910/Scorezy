...
import { getClassroomDetails } from "@/modules/student/server/actions/student.queries";
...
export default async function StudentClassroomDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  const session = await getSession();
  if (!session) return <SignInPage />;

  const resolvedParams = await params;
  const classroom = await getClassroomDetails(resolvedParams.id);
  if (!classroom) return notFound();

  // Filter only published quizzes (optional, but good practice)
  const activeQuizzes = classroom.quizzes.filter(q => q.status === "published");
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/5">
          <div className="space-y-1">
            ...
            <p className="text-[#a1a1aa] text-lg">
              Explore the assessments available in this classroom.
            </p>
          </div>
        </div>
        ...
        {activeQuizzes.length === 0 ? (
          <div className="text-center py-20 border border-white/5 border-dashed rounded-none bg-white/5 backdrop-blur-sm">
            ...
            <p className="mt-2 text-[#a1a1aa] max-w-xs mx-auto">
              Your teacher hasn't published any tests here yet.
            </p>
          </div>
        ) : (
